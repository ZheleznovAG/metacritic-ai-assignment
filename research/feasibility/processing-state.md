...
Логическая модель ниже достаточна для написания детерминированных тестов до реализации. Она не выбирает язык, базу данных, scheduler или механизм блокировки. Фактическая identity игры остаётся зависимостью `SPK-03`, контракт и порядок страниц — зависимостью `SPK-02`, а конкретные scheduler/lease capabilities публичной среды — зависимостью `SPK-06`.
...
| Game key | Абстрактная стабильная identity игры; конкретное правило обязан подтвердить `SPK-03` |
...
- `SPK-02` должен подтвердить фактическую пагинацию, порядок и признак exhaustion. До этого Cursor остаётся абстрактным checkpoint.
- `SPK-03` должен определить `game_key`. Без него инварианты уникальности сформулированы, но не реализуемы.
...
- [x] Зависимости `SPK-02`, `SPK-03` и `SPK-06` сохранены как ограничения, а не подменены догадками.
