...
[Очередь исправлений](docs/requirements/implementation_audit_6551e42.md#очередь-исправлений) закрыта локальными и Docker regressions. Текущий цикл — [повторная проверка IMP-02](docs/requirements/imp_02_revalidation.md): Elden Ring прошла два реальных ingest и публичную карточку; найдено и локально исправлено перемещение provenance сохранённой оценки при новом null. До завершения нужны hosted CI и VDS recheck исправленного candidate. Далее — собственное evidence IMP-03–05. Независимый SIM-EVAL-01 остаётся Ready. G4–G7 не закрыты. Локальная application DB не мигрировалась; до её запуска нужны [миграции](README.md), затем обычный успешный collection/build для verified current heads; исторические incomplete данные не реконструируются.
...
| [IMP-02](implementation_plan.md#imp-02) | IMP-01 | base | In progress | [Revalidation и provenance correction](docs/requirements/imp_02_revalidation.md): 2 live ingest, source/DB/public-card comparison, desktop/mobile screenshots; найденный retained-score provenance defect исправлен, 263 local application tests пройдены. Ожидаются hosted CI и VDS recheck исправленного candidate. |
