...
[Очередь исправлений](docs/requirements/implementation_audit_6551e42.md#очередь-исправлений) закрыта локальными и Docker regressions. [Повторная проверка IMP-01](docs/requirements/imp_01_revalidation.md) подтвердила hosted CI и обновлённый VDS на source SHA `8c76615`; миграции публичного стенда применены, его каталог пуст. Следующий цикл — IMP-02: реальный ingest → публичная карточка, затем собственное evidence IMP-03–05. Независимый SIM-EVAL-01 остаётся Ready. G4–G7 не закрыты. Локальная application DB не мигрировалась; до её запуска нужны [миграции](README.md), затем обычный успешный collection/build для verified current heads; исторические incomplete данные не реконструируются.
...
| [IMP-02](implementation_plan.md#imp-02) | IMP-01 | base | Ready | [R19/R21 corrections](docs/requirements/implementation_corrections_batch.md): замечания к коду устранены; dependency IMP-01 и hosted CI актуальной версии подтверждены. Следующий цикл: повторный реальный ingest → публичная карточка; каталог VDS пока пуст. Historical: [ingest/card evidence](docs/requirements/imp_02_review.md). |
