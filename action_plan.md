...
| Поле | Значение |
|---|---|
| Версия плана | Discovery baseline 0.3 |
| Дата | 2026-09-04 |
...
| Активная стадия | 2 — снятие критических неизвестных; `SPK-01` завершена решением `Ask`, следующая доступная задача `SPK-04` |
| Реализация | Не начата |
| Текущие блокеры | Для Metacritic-зависимой ветки нет письменного разрешения на автоматизированный доступ или одобренного интерфейса; дедлайн, бюджет и hosting также не заданы |
...
| 2. Снятие неизвестности | Реестр рисков и результаты Must-spikes | `G2` | `In progress` |
...
`SPK-04` — смоделировать время, партии и прогресс без зависимости от живого Metacritic. Параллельно владельцу нужно разрешить `Ask` из `SPK-01`; до этого `SPK-02` не запускается.
