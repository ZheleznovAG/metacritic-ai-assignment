...
Collection generation становится `complete`, только если `next` отсутствует, `totalResults` был стабилен на всех pages, каждый page завершён успешно, unresolved duplicates нет и unique fetched count равен reported total. Unique count считается по identity observations текущего job/generation, независимо от того, была ли Review version сохранена вчера; duplicate count равен fetched minus unique. Валидный нулевой total без полученных records даёт `empty`. Cursor loop, повтор ordered page identities, изменение route parameters, смена версии одного identity внутри обхода или расхождение counts дают `unstable`; изменение `totalResults` посреди обхода (быстрорастущий route новинки) сначала перезапускает generation с начального URL (не более трёх раз на job; прежние observations остаются историей, инвариант `reported = unique = fetched` без дублей не меняется) и только после лимита даёт `unstable`; транспортная ошибка — `retryable/failed`. Незавершённая generation не смешивается с предыдущей и не запускает summary. Прошлые observations и опубликованный summary сохраняются со stale-причиной до появления нового полного snapshot. Датированный внешний evidence и точные acceptance rules находятся в [`research/feasibility/metacritic-contract.md`](../research/feasibility/metacritic-contract.md) и [`reviews-pagination.json`](../research/feasibility/fixtures/metacritic/reviews-pagination.json).
...
3. Selection `1.1.0-sentiment` (расширение `1.0.0-candidate`, ниже) не берёт только первую/последнюю source page: внутри каждой платформы records сортируются по SHA-256 от policy version и review identity, затем платформы чередуются round-robin по стабильному `platform_slug`, который передаётся в selection pool. Так любой объём хранится полностью, а bounded sample воспроизводимо распределён по полному snapshot; репрезентативность этого правила остаётся quality-кандидатом. `REV-EVAL-01` сначала замораживает corpus examples, metric, threshold и hard invariants, `IMP-04` затем сравнивает candidate с более простым baseline без изменения oracle, а `HRD-04` проверяет финальную регрессию. Начиная с `1.1.0-sentiment` первые три отрицательных и три положительных review в этом детерминированном порядке резервируются, остальные слоты заполняются тем же порядком: иначе при перекосе пула (например, 36 положительных и 4 отрицательных) отбор по хэшу давал 0 отрицательных, и раздел Dislikes не имел опоры. Тональность берётся только из score (`< 50%` шкалы — отрицательный, `>= 75%` — положительный; шкала 100 для critic и 10 для user; без score — нейтральный), поэтому правка текста review по-прежнему не меняет выбранный набор. Смена версии политики меняет `contour_fingerprint`: сохранённые summary пересоздаются при следующем обходе игры в рамках бесплатной квоты (`delayed_capacity`), старые остаются видимыми как `stale`. Проверка: расширение оракула [`sentiment_cases.json`](../evals/review_selection/sentiment_cases.json) / [`score_sentiment.py`](../evals/review_selection/score_sentiment.py) (`INV-SENTIMENT-COVERAGE`, 7 кейсов; `1.0.0-candidate` проваливает 4 из 7).
...
`release_date`, `publishers` и `content_rating` читаются из JSON-LD `VideoGame` (`datePublished`, `publisher`, `contentRating`). Это необязательные метаданные: некорректное значение становится отсутствующим и не блокирует сохранение игры; слияние неразрушающее (`None`/пусто не затирает сохранённое), а непустой новый `publishers` заменяет прежний список. Существующие игры получают значения при следующем detail-fetch. Список игр умеет сортировку `sort=release` (новые выше, без даты в конце, затем по названию) и показывает дату выхода; значение по умолчанию — прежний порядок по Metascore (`ASM-15`), `sort` возвращается в ссылке «назад» ровно как пришёл. Игры с уже накопленными оценками добавляет `manage.py add_rated_games [--listing <год>/<сортировка>] [--limit N]` (по умолчанию `current-year/metascore`, лучшие игры года): почасовой планировщик идёт только по New Releases и «новейшим» листингам, где почти все игры без оценок; команда обходит листинг по оценке, пропускает уже сохранённые игры и вызывает тот же `ingest_game`, после чего worker обычным образом собирает отзывы и пишет резюме в рамках бесплатной квоты Groq. Чтобы не ждать, `manage.py backfill_game_metadata [--limit N] [--pause S]` один раз загружает страницу деталей каждой игры без `release_date` (без страниц оценок платформ, без задач на отзывы и summary, без daily candidate) и неразрушающе заполняет только эти три поля; его можно прерывать и перезапускать. Описание игры декодируется из HTML-сущностей один раз при разборе (`&bull;`, `&rsquo;`, `&nbsp;` в старых листингах Metacritic), ранее сохранённые строки исправляет data-миграция `catalog.0008`.
