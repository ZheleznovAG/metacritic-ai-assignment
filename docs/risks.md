...
| 2 | `SPK-02` | `R-EXT-03`, `R-EXT-04`, часть `R-SIM-01`, `R-TST-01` | Нужно подтвердить все поля и варианты страниц до модели данных и парсера | `SPK-01: Proceed*` | Ready |
...
| 3 | `SPK-05` | `R-AI-01`, `R-AI-02` | Eval требует репрезентативных отзывов или согласованного substitute | `SPK-02` либо допустимые samples | Planned — free Grok selected; waits for samples |
...
**Итог:** `RSK-01`, `SPK-01` и `SPK-04` завершены. Ответ владельца 2026-09-05 снял `SPK-01: Ask`, поэтому следующая задача `SPK-02` имеет статус `Ready`; free Grok выбран для будущей `SPK-05`. `SPK-06` остаётся `Blocked — Ask`, needed by `G2`.
