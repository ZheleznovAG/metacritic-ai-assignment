...
|---:|---|---|---|---|---|
| 1 | `SPK-01` | `R-EXT-01`, `R-EXT-02` | Без допустимого и воспроизводимого доступа основной источник может сделать весь Must нереализуемым | `RSK-01` | Verified — `Ask` |
| 2 | `SPK-02` | `R-EXT-03`, `R-EXT-04`, часть `R-SIM-01`, `R-TST-01` | Нужно подтвердить все поля и варианты страниц до модели данных и парсера | `SPK-01: Proceed*` | Blocked by `SPK-01: Ask` |
| 2 | `SPK-04` | `R-TIM-01`, `R-TIM-02` | Высокое влияние, но проверяется без сети на уже зафиксированных допущениях | `RSK-01` | Ready |
...
- **Текущая диспозиция:** `Open — mitigate`; техническое решение `Proceed with limitation`, но общий путь остановлен решением `R-EXT-02: Ask`.
...
- **Оценка после `SPK-01`:** `P=5`, `I=5`, `U=2`; Exposure `25`, Discovery `10`; приоритет `P0` из-за прямого блокирования Must.
...
- **Владелец:** `SPK-01`.
- **Evidence:** [`research/feasibility/metacritic-access.md`](../research/feasibility/metacritic-access.md).
- **Остаточный риск:** даже при разрешении нужно отдельно согласовать допустимую частоту, хранение отзывов/media и публичный показ; Terms может измениться.
- **Текущая диспозиция:** `Open — external decision`; решение `Ask`: требуется письменное разрешение Fandom, одобренный интерфейс либо согласованное изменение критерия задания.
...
**Итог:** `RSK-01` завершена; `SPK-01` также завершена с итогом `Ask`. Metacritic-зависимая `SPK-02` заблокирована до внешнего решения, а следующая доступная задача workflow — `SPK-04`.
