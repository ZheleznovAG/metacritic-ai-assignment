...
For an existing review database, apply `reviews.0004` and `summaries.0003` with `scripts/migrate.py` before starting the updated worker. It backfills immutable text/metadata version hashes while preserving review IDs, text hashes and history references. Pause writers during the upgrade; after metadata-only versions exist, the old text-only uniqueness constraint cannot represent the new data. Historical incomplete collections and lost source metadata are not repaired by this migration. See the [snapshot correction and upgrade limits](docs/requirements/imp_04_snapshot_correction.md).
